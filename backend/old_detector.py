...
STAGE1_PREPROC_PATH = os.environ.get("STAGE1_PREPROCESSOR_PATH", "/home/smart/Desktop/projects-last/projects/iwps/backend/backend/artifacts/stage_1/preprocessor.joblib")
STAGE1_FEATURES_PATH = os.environ.get("STAGE1_FEATURES_PATH", "/home/smart/Desktop/projects-last/projects/iwps/backend/backend/artifacts/stage_1/feature_names.json")  # optional JSON list

# ---- Stage-2 (multiclass) artifacts (optional) ----
STAGE2_CLF_PATH = os.environ.get("STAGE2_CLF_PATH", "/home/smart/Desktop/projects-last/projects/iwps/backend/backend/artifacts/stage_2/lgbm_multiclass_balanced.joblib")
STAGE2_PREPROC_PATH = os.environ.get("STAGE2_PREPROCESSOR_PATH", "/home/smart/Desktop/projects-last/projects/iwps/backend/backend/artifacts/stage_2/preprocessor.joblib")      # optional
STAGE2_FEATURES_PATH = os.environ.get("STAGE2_FEATURES_PATH", "/home/smart/Desktop/projects-last/projects/iwps/backend/backend/artifacts/stage_2/feature_names.json")         # optional (JSON list)
...
class Detector:
    ...
    def _save_packet_raw(self, row: Dict[str, Any], final_label: Optional[str],
                     proba_attack: Optional[float], proba_anomaly: Optional[float],
                     iface: Optional[str], stage2_info: Optional[Dict[str, Any]] = None):
        # Get a connection from the same SessionLocal the app uses
        s = SessionLocal()
        try:
            conn = s.get_bind()  # SQLAlchemy Engine/Connection
            is_pg = str(conn.url).startswith(("postgres://", "postgresql://"))
            ...
            if is_pg:
                ...
                # SQLite/MySQL fallback (JSON cast not needed)
                sql = text("""
                    INSERT INTO packets
                    (ts, iface, src_mac, dst_mac, bssid,
                    frame_len, channel_freq, datarate, signal_dbm,
            ...
            with conn.begin() as tx:
                ...
                if is_pg:
                    ...
                    # Optional: print(f"[db] inserted id={inserted_id}")
            self._saved += 1
